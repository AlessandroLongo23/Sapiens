...
- [x] Stripe live (account "New business", `acct_1QWnOJBX6jVQ5bX2`; rename it to Sapiens in the Dashboard): products Sapiens Lite / Base / Pro with monthly prices (4,99 / 19,99 / 59,99 €) and six-month prices at five monthly amounts (24,95 / 99,95 / 299,95 €), tax-inclusive, tax code `txcd_10000000`, lookup keys `sapiens_<plan>_monthly|semester`. Ids are in Vercel Production as `PUBLIC_STRIPE_PRICE_*` and `PUBLIC_STRIPE_PRICE_*_SEMESTER`, so the semester toggle is live.
...
- [ ] Stripe live webhook: to create with the CLI once the live login is approved (`/api/stripe/webhook` on the production host, API version 2026-08-26.dahlia, events `checkout.session.completed`, `customer.subscription.*`, `invoice.paid`, `invoice.payment_failed`); its secret then goes to Vercel Production as `STRIPE_WEBHOOK_SECRET`.
- [ ] Stripe live key for Production: create a restricted key in the Dashboard (Developers → API keys → Create restricted key) with Checkout Sessions, Customers, Subscriptions and Customer portal set to Write and everything else None; put it in the local `.env` as `STRIPE_SECRET_KEY_LIVE` and it gets pushed to Vercel as a sensitive variable. The existing Production `STRIPE_SECRET_KEY` value in Vercel is unknown (encrypted) and is replaced.
