...
- Phone suite (`tests/e2e/mobile.spec.ts`, added 2026-09-07) on two extra projects, iPhone 14 in WebKit and Pixel 7 in Chromium: tab bar, menu sheet, full-screen search, header that hides on scroll, lesson section bar, table of contents sheet, assistant sheet, selection menu kept on screen, the exercise runner with a subscribed throwaway user, tutor filters and the request form as sheets, and no sideways overflow on any page. `E2E_BASE_URL=http://localhost:3000 E2E_SKIP_STRIPE_LISTEN=1 npx playwright test tests/e2e/mobile.spec.ts` runs it against the dev server; `npm run screenshots:mobile` captures the main pages on a chosen set of devices.
...
- [ ] Set `PUBLIC_GSC_VERIFICATION` (Google Search Console HTML-tag token) in Vercel, or drop the verification file into `public/`.
...
- [ ] Review the ten subject texts in `src/lib/content/subject-copy.ts` and the ten FAQ answers in `src/app/(site)/faq/page.tsx`.
