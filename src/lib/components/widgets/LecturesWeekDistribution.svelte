<script>
	...
	let svg;
	let width;
	let height = 350; // Fixed height for the chart area
	...
	function createChart() {
		...
		// Add circular grid lines with better spacing
		const maxCount = ceilToMultiple(d3.max(weekdayDistribution, d => d.count), 5) || 1;
		const tickCount = 3; // Number of concentric circles
		...
		g.selectAll('.grid-circle')
			...
			.attr('stroke-dasharray', '10, 10'); // Dotted lines
			
		// Add central white circle to create empty space in the middle
		g.append('circle')
			.attr('r', 30)
			.attr('fill', $themeStore === 'dark' ? '#121212' : 'white')
			.attr('stroke', 'none');
			
		// Add labels for grid circles in better positions
		g.selectAll('.grid-label')
			.data(gridCircles)
			.join('text')
			.attr('class', 'grid-label')
			.attr('y', d => -radiusScale(d) - 7) // Position above the circle
			.attr('x', 0) // Offset horizontally for better readability
			.attr('dy', '0.35em')
		...
		// Add day labels around the circle
		g.selectAll('.day-label')
			.data(weekdayDistribution)
			.join('text')
			.attr('class', 'day-label')
			.each(function(d) {
				// Calculate the angle at the center of the bar
				const angle = angleScale(d.dayIndex) + angleScale.bandwidth() / 2 - Math.PI;
				
				// Position text at the correct distance from the center
				const labelRadius = radius + 30; // A bit further from the chart edge
				const x = labelRadius * Math.cos(angle);
				const y = labelRadius * Math.sin(angle);
				
				// Set text-anchor based on position around the circle
				let textAnchor;
				if (angle > -Math.PI/4 && angle < Math.PI/4) {
					...
					textAnchor = 'middle'; // Top or bottom
				}
				
				// Apply the calculated attributes
				d3.select(this)
					.attr('x', x)
					.attr('y', y)
    ...
    // Create a color scale that transitions around the circle
	const colorScale = d3.scaleSequential()
		.domain([7, 0]) // 7 days of the week
	...
	const bars = g.selectAll('.radial-bar')
		...
		.attr('d', d => {
			...
			const innerRadius = 30; // Start bars from inner circle, not center
			...
			const arc = d3.arc()
				...
				.padAngle(0.03) // Increase space between bars
				.padRadius(radius)
				.cornerRadius(4); // Round the corners
		...
		bars.on('mouseover', function(event, d) {
			...
			tooltipData = {
				day: d.fullDay, // Use the full day name in tooltip
				count: d.count,
				x: x,
				y: y
			};
...
<div class="w-full flex flex-col overflow-hidden bg-white border border-[#E5E7EB] dark:bg-[#121212] dark:border-[#2A2A2A] rounded-lg shadow-base dark:shadow-md transition-all hover:shadow-md dark:hover:shadow-glow">
	<div class="px-5 py-4 sm:p-6 border-b border-[#E5E7EB] dark:border-[#2A2A2A] flex items-center">
		<div class="p-2.5 mr-4 rounded-md bg-[#FEF3C7] dark:bg-[#1E1E1E]">
			<ls.Calendar class="w-5 h-5 text-[#F5BD0B]" />
		</div>
		<div class="flex-1">
