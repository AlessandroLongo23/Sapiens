<script>
    ...
    function toggleSelect(nodeId, event) {
        ...
        if (newSelected.has(nodeId)) {
            ...
            // When deselecting, recursively deselect all children
            ...
            // When selecting, recursively select all children
            if (node.children && node.children.length > 0) {
                recursiveSelect(node, newSelected);
            }
        }
        
        // Dispatch event to parent component with additional info for parent/child recursion
    ...
    // Helper function to recursively deselect all children
