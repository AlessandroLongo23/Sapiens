import { writable } from "svelte/store";

export const content = {
    superiori: {
        informatica: {
            3: {
                topics: {
                    "foglio-di-calcolo": {
                        "title": "Foglio di Calcolo",
                        "description": "Il foglio di calcolo è un programma che permette di creare e modificare tabelle.",
                        "icon": "/subjectsIcons/spreadsheet.png",
                    }
                }
            }
        },
        matematica: {
            1: {
                topics: {
                    "numeri-naturali": {
                        title: "Numeri Naturali",
                        description: "I numeri naturali sono i numeri che si usano per contare.",
                        icon: "/subjectsIcons/numeri-naturali.png",
                        subtopics: {
                            "operazioni-e-proprieta": {
                                title: "Le quattro Operazioni",
                                description: "Somma, sottrazione, moltiplicazione e divisione, e le loro proprietà.",
                                icon: "",
                            },
                            "potenze": {
                                title: "Le Potenze",
                                description: "La potenza e le sue proprietà.",
                                icon: "",
                            },
                            "mcm-MCD": {
                                title: "mcm e MCD",
                                description: "Il minimo comune multiplo e il massimo comune divisore di due o più numeri.",
                                icon: "",
                            }
                        }
                    },
                    "numeri-razionali": {
                        title: "Numeri Razionali",
                        description: "I numeri razionali sono i numeri che si usano per misurare.",
                        icon: "subjectsIcons/numeri-razionali.png",
                        subtopics: {
                            "confronto frazioni": {
                                title: "Confronto frazioni",
                                description: "Confronto frazioni.",
                                icon: "",
                            },
                            "potenze esponente negativo": {
                                title: "Potenze esponente negativo",
                                description: "Potenze con esponente negativo.",
                                icon: "",
                            },
                            "conversione": {
                                title: "Conversione",
                                description: "Conversione di numeri dalla forma frazionaria alla forma decimale e viceversa.",
                                icon: "",
                            }
                        }
                    },
                    "insiemi-e-logica": {
                        title: "Insiemi e logica",
                        description: "Gli insiemi e le loro operazioni.",
                        icon: "/subjectsIcons/insiemi.png",
                    },
                    "monomi-e-polinomi": {
                        title: "Monomi",
                        description: "I monomi e le loro operazioni.",
                        icon: "/subjectsIcons/monomi.png",
                    }
                }
            }
        },
    },
    università: {
        "analisi-I": {
            title: "Analisi I",
            description: "L'analisi I è la parte della matematica che studia i numeri, le figure geometriche e le relazioni tra di loro.",
            icon: "subjectsIcons/analisi-I.png",
        }
    }
}

export let selectedTopic = writable(null);