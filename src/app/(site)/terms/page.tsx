...
export const metadata: Metadata = pageMetadata({
	...
	description: "Termini e condizioni d'uso di Sapiens: chi può registrarsi, cosa è gratuito, il piano Studio, la prova gratuita, pagamenti, diritto di ripensamento, disdetta e uso dei contenuti.",
	path: '/terms'
});
...
const saleMonths = SCHOOL_YEAR_PASS.saleMonths.map((m) => MONTHS[m - 1]).join(' e ');
const passEnds = `${SCHOOL_YEAR_PASS.endDay} ${MONTHS[SCHOOL_YEAR_PASS.endMonth - 1]}`;
...
export default function TermsPage() {
	return (
		<Prose>
			...
			<p>Queste condizioni regolano l&apos;uso del sito {SITE_NAME} e del piano Studio. Usando il sito le accetti; creando un account le accetti espressamente. Sono scritte in modo che anche uno studente possa capirle.</p>
			...
			<p>{SITE_NAME} pubblica materiale didattico per la scuola media, la scuola superiore e l&apos;università: teoria, formulari, esercizi interattivi con correzione automatica, flashcard, un assistente di studio basato sull&apos;intelligenza artificiale (Sapiens AI) e lo Zaino, dove prendere appunti. Il catalogo cresce nel tempo: le lezioni non ancora scritte sono indicate come &quot;in arrivo&quot;.</p>

			<h2>3. Contenuti gratuiti</h2>
			<p>La teoria e i formulari sono consultabili gratuitamente, senza registrazione e senza limiti di tempo. Non serve un account per studiare. Con un account gratuito (piano Free) hai anche una sessione di esercizi al giorno e un quaderno nello Zaino.</p>
			...
			<h2>5. Piano Studio, prezzi e prova gratuita</h2>
			<p>Il piano a pagamento è Studio. I prezzi sono finali, IVA inclusa, senza costi aggiuntivi:</p>
			<ul>
				<li>abbonamento mensile: {formatPrice(STUDIO.price, STUDIO.currency)} al mese;</li>
				<li>Studio fino al {passEnds}: {formatPrice(SCHOOL_YEAR_PASS.price, STUDIO.currency)} in un solo pagamento, in vendita solo a {saleMonths}. Dà accesso a Studio fino al {passEnds} dello stesso anno e non si rinnova.</li>
			</ul>
			<p>Sono riservati a Studio:</p>
			<ul>
				{PREMIUM.map((f) => (
			...
			<p>Se sei un consumatore, hai 14 giorni dal primo addebito, o dal pagamento di Studio fino al {passEnds}, per ripensarci (art. 52 del Codice del consumo, D.lgs. 206/2005): scrivici dalla pagina <a href="/contacts">Contatti</a> e ti rimborsiamo per intero l&apos;importo di quel periodo. Non serve indicare un motivo. Per i rinnovi successivi vale la disdetta descritta al punto 8.</p>

			<h2>8. Disdetta</h2>
			<p>Puoi disdire l&apos;abbonamento mensile in qualsiasi momento dalla pagina <a href="/subscription">Il tuo abbonamento</a> (&quot;Gestisci abbonamento&quot;). Il piano resta attivo fino alla fine del mese già pagato e non si rinnova; il mese in corso non viene rimborsato, salvo il diritto di ripensamento del punto 7. Studio fino al {passEnds} non va disdetto: finisce da solo. Puoi riattivare Studio quando vuoi.</p>
			...
			<h2>11. Disponibilità del servizio e correttezza dei contenuti</h2>
			...
			<h2>13. Chiusura dell&apos;account da parte nostra</h2>
			<p>Possiamo sospendere o chiudere un account in caso di violazione di queste condizioni, uso fraudolento o richiesta dell&apos;autorità, dopo averti avvisato via email quando possibile. In caso di chiusura per nostra iniziativa senza tua violazione, rimborsiamo la parte di piano non goduta.</p>
			...
			<h2>15. Modifiche a queste condizioni</h2>
