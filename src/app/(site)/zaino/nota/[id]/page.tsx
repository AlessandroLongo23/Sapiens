...
import { getNote, getNotePaper, getNoteStickers, getNotebook, listNotebooks } from '@/lib/server/zaino';
...
export default async function NotePage({ params }: { params: Promise<{ id: string }> }) {
	...
	if (!note) notFound();
	const [notebook, notebooks, stickers, paper] = await Promise.all([
		getNotebook(supabase, user.id, note.notebook_id),
		listNotebooks(supabase, user.id),
		getNoteStickers(supabase, user.id, note.id),
		getNotePaper(supabase, user.id, note.id)
	]);
	return <NoteEditor note={note} notebookTitle={notebook?.title ?? 'Quaderno'} notebooks={notebooks} stickers={stickers} paper={paper} />;
}
